...
void SystemClock_Config(void)
{
  ...
  */
  /* HSI48 added alongside HSI/PLL: RNG (MX_RNG_Init, below) is clocked from
   * it -- it's the internal 48 MHz RC used for USB/RNG, unrelated to the
   * SYSCLK/PLL chain configured right after this. */
...
/**
  * @brief RNG Initialization Function -- hardware TRNG, clocked from HSI48
  * (see SystemClock_Config above). Used to seed the TLS/crypto library's
  * NX_RAND() (see nx_user.h / Core/Src/nx_rand_hw.c) instead of falling back
  * to libc's rand(), which nothing in this project ever seeds and which
  * newlib-nano starts from a fixed default -- i.e. the exact same "random"
  * sequence on every single boot. Not acceptable for anything TLS uses it
  * for (ClientHello random, RSA blinding, etc).
  * @param None
  * @retval None
  */
static void MX_RNG_Init(void)
