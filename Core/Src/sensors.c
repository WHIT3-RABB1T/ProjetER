 ...
 * Sensors_ReadJSON() to decide what to include. A 0 here means that
 * specific chip is simply left out of every future reading -- one bad
 * sensor never takes the others down with it. */
 ...
 * behaviour -- Sensors_ReadJSON()'s caller only ever uses the returned
 * length, never assumes every key made it in.
 *
...
uint32_t Sensors_ReadJSON(char *buf, uint32_t buf_size)
{
    ...
    /* --- Environmental: {"env":{"hts221":{...},"lps22hh":{...}}} --- */
    used = append(buf, buf_size, used, "\"env\":{");
    if (s_hts221_ok)
    {
        float temp_c = 0.0f;
        float hum_rh = 0.0f;
        int32_t t_ret = BSP_ENV_SENSOR_GetValue(0, ENV_TEMPERATURE, &temp_c);
        int32_t h_ret = BSP_ENV_SENSOR_GetValue(0, ENV_HUMIDITY, &hum_rh);

        if ((t_ret == BSP_ERROR_NONE) || (h_ret == BSP_ERROR_NONE))
        {
            uint8_t need_comma = 0;
            long whole, frac;

            used = append(buf, buf_size, used, "\"hts221\":{");
            if (t_ret == BSP_ERROR_NONE)
            {
                float_to_fixed2(temp_c, &whole, &frac);
                used = append(buf, buf_size, used, "\"temp_c\":%ld.%02ld", whole, frac);
                need_comma = 1;
            }
            if (h_ret == BSP_ERROR_NONE)
            {
                float_to_fixed2(hum_rh, &whole, &frac);
                used = append(buf, buf_size, used, "%s\"hum_rh\":%ld.%02ld", need_comma ? "," : "", whole, frac);
            }
            used = append(buf, buf_size, used, "}");
            any_env = 1;
        }
    ...
    if (s_lps22hh_ok)
    {
        ...
        int32_t t_ret = BSP_ENV_SENSOR_GetValue(1, ENV_TEMPERATURE, &temp_c);
        int32_t p_ret = BSP_ENV_SENSOR_GetValue(1, ENV_PRESSURE, &press_hpa);

        if ((t_ret == BSP_ERROR_NONE) || (p_ret == BSP_ERROR_NONE))
        {
            uint8_t need_comma = 0;
            long whole, frac;

            used = append(buf, buf_size, used, "%s\"lps22hh\":{", any_env ? "," : "");
            if (t_ret == BSP_ERROR_NONE)
            {
                float_to_fixed2(temp_c, &whole, &frac);
                used = append(buf, buf_size, used, "\"temp_c\":%ld.%02ld", whole, frac);
            ...
            if (p_ret == BSP_ERROR_NONE)
            {
                float_to_fixed2(press_hpa, &whole, &frac);
                used = append(buf, buf_size, used, "%s\"press_hpa\":%ld.%02ld", need_comma ? "," : "", whole, frac);
            }
            used = append(buf, buf_size, used, "}");
            any_env = 1;
        }
    ...
    /* --- Motion: {"motion":{"accel_mg":{...},"gyro_mdps":{...},"mag_mgauss":{...}}} ---
     * Axes are already plain int32_t (mg / mdps / mgauss) -- no float
     * involved here at all. */
    used = append(buf, buf_size, used, ",\"motion\":{");
    if (s_accel_gyro_ok)
    {
        ...
        {
            used = append(buf, buf_size, used, "%s\"accel_mg\":{\"x\":%ld,\"y\":%ld,\"z\":%ld}",
                          any_motion ? "," : "", (long)axes.xval, (long)axes.yval, (long)axes.zval);
            any_motion = 1;
        }
        if (BSP_MOTION_SENSOR_GetAxes(0, MOTION_GYRO, &axes) == BSP_ERROR_NONE)
        {
            used = append(buf, buf_size, used, "%s\"gyro_mdps\":{\"x\":%ld,\"y\":%ld,\"z\":%ld}",
                          any_motion ? "," : "", (long)axes.xval, (long)axes.yval, (long)axes.zval);
            any_motion = 1;
        }
    }
    if (s_mag_ok)
    {
        ...
        {
            used = append(buf, buf_size, used, "%s\"mag_mgauss\":{\"x\":%ld,\"y\":%ld,\"z\":%ld}",
                          any_motion ? "," : "", (long)axes.xval, (long)axes.yval, (long)axes.zval);
            any_motion = 1;
        }
    }
    ...
    /* --- Light: {"light":{"als_raw":N,"white_raw":N}} -- raw register
     * counts, not calibrated lux (BSP_LIGHT_SENSOR_GetValues() returns the
     * two channels straight from the sensor with no lux conversion
     * applied); still real per-poll ambient-light data either way. */
    if (s_light_ok)
    {
        ...
        {
            used = append(buf, buf_size, used, ",\"light\":{\"als_raw\":%lu,\"white_raw\":%lu}",
                          (unsigned long)light_values[0], (unsigned long)light_values[1]);
        }
    }

    /* --- Ranging: "ranging_mm":[...] -- one entry per zone (4x4 profile
     * => 16 zones, row-major), first target's distance in mm, or -1 where
     * the BSP reports that zone's target status as not-OK (BSP's own
     * Status[] convention: 0 = OK, per b_u585i_iot02a_ranging_sensor.h). */
    if (s_ranging_ok)
    {
        /* static: RANGING_SENSOR_Result_t's ZoneResult[] array is always
         * sized for the max 64 zones (RANGING_SENSOR_MAX_NB_ZONES,
         ...
         * this one thread ever calls Sensors_ReadJSON(). */
        static RANGING_SENSOR_Result_t result;

        if (BSP_RANGING_SENSOR_GetDistance(VL53L5A1_DEV_CENTER, &result) == BSP_ERROR_NONE)
        {
            uint32_t i;

            used = append(buf, buf_size, used, ",\"ranging_mm\":[");
    ...
    /* append() never writes past buf_size, but make sure the string is
     * always properly terminated even if the last append() got clamped
     * mid-token (truncated JSON is still a valid, parseable-up-to-a-point
     * C string, never a buffer overrun). */
