...
| `tools/` | Le serveur HTTPS côté PC et le tableau de bord web — voir `tools/README.md`. | Projet |
...
- **Découverte automatique du serveur** (`Discover_ServerIP()`,
  `app_netxduo.c`, voir aussi `tools/README.md` §4) : la carte diffuse une
...
`tools/README.md` §9 côté serveur) — c'est ce qui permet à la carte
...
voir `tools/README.md` §2 pour pourquoi le serveur doit s'y adapter
explicitement.
...
- `tools/README.md` — le serveur HTTPS côté PC, l'API du tableau de bord,
