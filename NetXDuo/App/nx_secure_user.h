...
/* Enabled here: tools/http_server.py serves a self-signed cert (there's no
   real CA in this project's chain of trust) -- the board must still add it
   to its trusted store (see tls_setup_callback in app_netxduo.c) for the
   handshake to succeed; this macro alone does not skip verification. */
