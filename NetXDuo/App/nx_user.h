...
/* Default NX_RAND (a few lines below, in nx_api.h) is plain libc rand(),
   which nothing in this project ever seeds -- same sequence every boot.
   TLS (via NX_CRYPTO_RAND(), which just aliases NX_RAND -- see
   crypto_libraries/inc/nx_crypto.h) needs a real source of entropy, so
   route it through the STM32U5's hardware TRNG instead
   (Core/Src/nx_rand_hw.c). This #define must come before nx_api.h's own
   "#ifndef NX_RAND" fallback -- it does, since nx_user.h is pulled in from
   nx_port.h, which nx_api.h includes well above that point. */
extern unsigned int nx_rand_hw(void);
#define NX_RAND nx_rand_hw
